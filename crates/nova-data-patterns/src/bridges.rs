...
// ---------------------------------------------------------------------------
// SqlCqrsBridge – wraps nova-sql ReadWritePool for CommandStore + QueryStore
// ---------------------------------------------------------------------------

#[cfg(feature = "sql-bridge")]
pub struct SqlCqrsBridge {
    pool: nova_sql::ReadWritePool,
    initialized: AtomicBool,
}

#[cfg(feature = "sql-bridge")]
impl SqlCqrsBridge {
    pub fn new(pool: nova_sql::ReadWritePool) -> Self {
        Self {
            pool,
    ...
    pub async fn init(&self) -> Result<(), CqrsError> {
        ...
        use nova_sql::ConnectionTrait;
        db.execute(nova_sql::Statement::from_string(backend, create_events))
            .await
            .map_err(|e| CqrsError::Backend(e.to_string()))?;
        db.execute(nova_sql::Statement::from_string(
            backend,
            create_projections,
        ))
        .await
        ...
        info!("SqlCqrsBridge: tables initialized");
        Ok(())
    }
...
impl CommandStore for SqlCqrsBridge {
    async fn save_event(&self, event: StoredEvent) -> Result<(), CqrsError> {
        ...
        use nova_sql::ConnectionTrait;
        db.execute(nova_sql::Statement::from_sql_and_values(
            backend,
            sql,
            vec![
    ...
    async fn get_events(&self, aggregate_id: &str) -> Result<Vec<StoredEvent>, CqrsError> {
        ...
        use nova_sql::ConnectionTrait;
        let rows = db
            .query_all(nova_sql::Statement::from_sql_and_values(
                backend,
                sql,
...
impl QueryStore for SqlCqrsBridge {
    async fn get_projection_raw(&self, key: &str) -> Result<Option<JsonValue>, CqrsError> {
        ...
        let sql = match backend {
            nova_sql::DbBackend::Sqlite => "SELECT value FROM nova_projections WHERE pkey = ?1",
...
pub struct NoSqlQueryStore {
    nosql: nova_nosql::NovaNoSql,
    collection: String,
}

#[cfg(feature = "nosql-bridge")]
impl NoSqlQueryStore {
    pub fn new(nosql: nova_nosql::NovaNoSql) -> Self {
        Self {
            nosql,
...
mod sql_bridge_tests {
    ...
    async fn sqlite_round_trip_events_and_projections() {
        let db = nova_sql::Database::connect("sqlite::memory:")
            .await
            .expect("sqlite in-memory connection");

        use std::sync::Arc;
        use tokio::sync::RwLock;
        let pool = nova_sql::ReadWritePool::new(db, Arc::new(RwLock::new(Vec::new())));
