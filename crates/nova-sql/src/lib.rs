pub use sea_orm::{ConnectionTrait, Database, DatabaseConnection, EntityTrait, Schema, DbBackend, Statement};
pub use sea_orm_migration::prelude::*;
use nova_core::{NovaPlugin, axum::Router, axum::Extension, async_trait};

pub struct NovaSql {
    pub db: DatabaseConnection,
    sync_tasks: Vec<Box<dyn for<'a> Fn(&'a NovaSql) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send + 'a>> + Send + Sync>>,
}
...
impl NovaSql {
    pub async fn connect(url: &str) -> Self {
        ...
        Self { db, sync_tasks: Vec::new() }
    }

    pub fn add_entity<E>(mut self) -> Self 
    where E: EntityTrait + 'static 
    {
        self.sync_tasks.push(Box::new(|sql| {
            Box::pin(sql.sync_entity::<E>())
        }));
        self
    }
        ...
        if existing_columns.is_empty() {
            ...
            for column in table_create_stmt.get_columns() {
                ...
                if !existing_columns.contains(&col_name) {
                    println!("✨ Adding missing column '{}' to '{}'", col_name, table_name);
                    ...
                    self.db.execute(alter_stmt).await.expect("Failed to alter table");
                }
            }
        }
    }
    ...
    async fn get_table_columns(&self, table_name: &str) -> Vec<String> {
        ...
        match self.db.get_database_backend() {
            DbBackend::Sqlite => {
                ...
                let res = self.db.query_all(Statement::from_string(DbBackend::Sqlite, sql)).await.unwrap();
                for row in res {
            ...
            DbBackend::Postgres => {
                ...
                let res = self.db.query_all(Statement::from_sql_and_values(
                    DbBackend::Postgres, 
                    sql, 
                    vec![table_name.into()]
                )).await.unwrap();
    ...
    pub async fn run_migrations<M>(&self) where M: MigratorTrait {
